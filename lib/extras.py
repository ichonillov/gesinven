...
class ExtrasWindow(ttk.Frame, ThemeEngine):
    ...
    def __init__(self, master):
        ...
        master.title("Sistema de Gestión de Inventario \ Extras")
        ...
        # Heading Label
        heading_label = ttk.Label(ch_pass_frame, text="CAMBIAR CONTRASEÑA",
                                 font="Arial 20 bold", foreground='#4eacfe')
        heading_label.pack(side='top', pady=25)
        # Current Password
        current_password_label = ttk.Label(ch_pass_frame, text="Contraseña Actual",
                                            font="Arial 17 bold")
        ...
        # New Password
        new_password_label = ttk.Label(ch_pass_frame, text="Nueva Contraseña",
                                         font="Arial 17 bold")
        ...
        heading_label = ttk.Label(theme_frame, text="CAMBIAR TEMA",
                                 font="Arial 20 bold", foreground='#4eacfe')
        heading_label.pack(side='top', pady=25)
        ...
        if new_password_fetch != "":
            if current_password_fetch == decrypted_password:
                if current_password_fetch == new_password_fetch:
                    tk.messagebox.showwarning("Alerta", "La contraseña actual es igual a la nueva contraseña")
                    self.new_password_entry.delete(0, 'end')
                else:
                    new_password = Crypting.Encrypt(new_password_fetch)
                    self.db_obj.change_password(new_password)
                    tk.messagebox.showinfo("Satisfactorio", "Constraseña actualizada exitosamente")
                    self.current_password_entry.delete(0, 'end')
                    self.new_password_entry.delete(0, 'end')
            else:
                tk.messagebox.showerror("Error", "La contraseña actual incorrecta")
                self.current_password_entry.delete(0, 'end')
                # self.new_password_entry.delete(0, 'end')
        else:
            tk.messagebox.showerror("Error", "El nuevo password no puede estar vacío")
